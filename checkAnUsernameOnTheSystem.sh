...
numberNames=$(wc -l /etc/passwd|cut -d/ -f1)      # numberNames=echo "${#names[*]}"    ||   numberNames=echo ${#names[*]}
   ...
   read User_2
     ...
     do
        ...
        then
             ...
             exit                    # using 'exit', the function exits all nested cycles ('for' and 'while' cycles); using 'break' instead of 'exit', the function exits the current nested cycle only (' for' cycle) 
